...
#CONFIGURATION 
#NUM_MONGOD=`cat /home/ubuntu/DockerWork/num_mongod`
source /home/ubuntu/DockerWork/num_mongod
...
runCont(){
	...
	if [ $TYPE = 'mongod' ]; then
		#MONGOD
		CPUset=$(( $i - 1 ))
        	PORT=`/home/ubuntu/DockerWork/getPort.sh $NUMBER`
        	echo PORT "$PORT"
		if [ $CGROUP -eq 0 ]; then
			...
			docker run --hostname $NAME --cpuset-cpus=$CPUset --memory="$MEM"m  --name $NAME -p $PORT:27017 -v $MONGODDIR$NUMBER:/data mongodb $MONGODCMD --replSet rs$i
		fi
	elif [ $TYPE = 'configsvr' ]; then
		#MONGO CONFIG SERVER
		if [ $CGROUP -eq 0 ]; then
			...
			docker run --hostname $NAME --cpuset-cpus=13 --memory="$MEMc"m --name $NAME -p $PORT:27019 -v $MONGOCFG:/data mongodb $MONGODCMD --configsvr
		fi
	else 
		#MONGOS
		if [ $CGROUP -eq 0 ]; then
			...
			docker run --hostname $NAME --cpuset-cpus=14,15 --memory="$MEMs"m --name $NAME -p $PORT:27017 -v $MONGOSDIR$i:/data mongodb $MONGOSCMD $5 
		fi
	fi	
...
startRS(){
	...
	#	createdir mongod $i
		...
		runCont "rs$i-srv1" "$PORTRANGE$i" "$i" 'mongod' > /home/ubuntu/DockerWork/log/mongodRS1-$i.log 2>&1 &
...
startConfigSvr(){
	#CONFIG SERVER
	runCont cfgsrv1 27019 0 configsvr > /home/ubuntu/DockerWork/log/cfgsvr.log 2>&1 &
	exit 0
}
...
startMongos(){
	...
	runCont mongos 27017 0 mongos $1:$2 > /home/ubuntu/DockerWork/log/mongos.log 2>&1 & 
	exit 0
}
...
createenv(){
	...
	sudo rm -rf $WORKDIR/sh* $WORKDIR/configsvr
	for i in `seq 1 $NUM_MONGOD`;
	do
		createdir mongod $i
	done

...
runCont(){
	...
	if [ $TYPE = 'mongod' ]; then
		#MONGOD
		if [ $CGROUP -eq 0 ]; then
			docker run --hostname $NAME --name $NAME -p $PORT:27017 -v $MONGODDIR$NUMBER:/data mongodb $MONGODCMD  
		else
			echo "Running with cgroups"
			docker run --hostname $NAME --cpu-shares=$CPU --memory="$MEM"m  --name $NAME -p $PORT:27017 -v $MONGODDIR$NUMBER:/data mongodb $MONGODCMD 
		fi
	elif [ $TYPE = 'configsvr' ]; then
		#MONGO CONFIG SERVER
		if [ $CGROUP -eq 0 ]; then
...
startRS(){
	...
	#	createdir mongod $i
		...
		runCont "rs$i-srv1" "$PORTRANGE$i" "$i" 'mongod' > log/mongodRS1$i.log 2>&1 &
		#docker run --name "rs$i-srv1" -p "$PORTRANGE$i":27017 -v "/space/data/db$i":/data mongodb mongod -f /etc/mongod.conf   &
		#> /dev/null 2>&1 &

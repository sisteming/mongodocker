...
startRS(){
	...
	#	createdir mongod $i
		...
		echo "rs$i-srv3 $PORTRANGE$i $i 'mongod'"
		runCont "rs$i-srv3" "$PORTRANGE$i" "$i" 'mongod' > log/mongod$i-rs3.log 2>&1 &
		#docker run --name "rs$i-srv3" -p "$PORTRANGE$i":27017 -v "/space/data/db$i":/data mongodb mongod -f /etc/mongod.conf --replSet rs$i  &
		#> /dev/null 2>&1 &
